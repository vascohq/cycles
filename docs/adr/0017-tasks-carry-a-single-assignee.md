...
- **MCP assignment — deferred in v1, now shipped.** Originally reader-only: `assigneeId` surfaced in reads but `upsert_task` couldn't set it, to keep Clerk out of the pure-Liveblocks write path. Now implemented: `upsert_task` takes `assignee` (an **email or userId** — _not_ a display name, which is ambiguous), resolved server-side in the tool handler (`resolveAssigneeRef` over `getOrganizationUsers`) so the writer stays Liveblocks-only; `''` unassigns, omit leaves unchanged (ADR 0011 partial). A `list_members` tool exposes `{userId, name, email}` for discovery, and `move_task` reorders a task relative to a sibling (`before`/`after`) via `LiveList.move`, mirroring the in-app drag. The resolver matches email/userId only — never a name.
