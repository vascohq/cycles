...
- **Latest-only is enforced on both paths.** The UI only renders the affordance on the newest card; MCP's `delete_update` validates the id is the latest update for its pitch and errors otherwise — it is a data-integrity invariant, not a UI policy.
