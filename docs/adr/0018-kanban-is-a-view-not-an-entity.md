...
- **The anchor must be a card on the same pitch, and `moved` tells the truth.** Order is one cycle-wide list, so an anchor from another board is silently meaningless — the card lands next to a card its own column never shows, sometimes not moving at all. That case is rejected rather than reported as success. `moved` reports whether the *position* changed, so "already there" (false, no error) is distinguishable from "reordered" — a UI can see the outcome on screen, an agent only has the response. Batched ops carry that outcome too: a batch is how a whole board gets re-ranked, so swallowing `moved` there would blind exactly the caller that needs it.
- **Card status is validated in the writer, not just at the tool.** `batch` forwards params un-validated (`z.record`), so a tool's zod enum is not a guarantee downstream: an unknown status would persist and then throw on every column lookup, taking the board down for the whole room until another write repaired it. Both `move_task` and `upsert_task` check the enum at the writer — the one door every entry path passes through — and `cardStatus` additionally treats an out-of-enum *stored* value as absent, because stored data outlives the code that wrote it.
